...
public partial class TacticTool
{
    ...
    private async Task ChangeTool(IconType tool)
    {
        await TacticToolService.ChangeEditMode(tool);
    }

    private async Task SelectedUnit(string unit)
    {
        await TacticToolService.ChangeUnit(unit);
    }

    private async Task OnColorChanged(string color)
    {
        await TacticToolService.ChangeColor(color);
    }
}
